...
class DashboardRepository {
    ...
    async getNotifications() {
        try {
            ...
            const currentDate = new Date();
            currentDate.setDate(currentDate.getDate() - 7); // Get the date one week ago

            const targetDayOfWeek = 0; // Replace with your desired day of the week (0 for Sunday, 1 for Monday, etc.)

            // Calculate the start date of the current week based on the target day of the week
            ...
            const whereClause = {
                createdAt: {
                    [Op.between]: [currentWeekStartDate, currentDate], // Date range from the start of the week to one week ago
                },
            } as unknown as Record<string, any>;
    ...
    async getAdvertising(type: string, target: string) {
        ...
        try {
            if (type == 'day') {
                ...
                return await this.advertisingModel.count({ where: whereClause });
            } else if (type == 'week') {
                const currentDate = new Date();
                const currentDayOfWeek = currentDate.getDay(); // 0 (Sunday) to 6 (Saturday)
                const currentWeekStartDate = new Date(currentDate);
                ...
                for (let day = 0; day <= 6; day++) {
                    ...
                    const users: any = await this.advertisingModel.count({ where: whereClause });
